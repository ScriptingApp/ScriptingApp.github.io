...
Attempts to read the result as an image.

Returns a file path to the image, or `null` if unavailable.
...
Attempts to read the result as a video.

Returns a file path to the video, or `null` if unavailable.
...
The Photos module provides a clean and reliable abstraction over iOS photo and camera capabilities, covering capture, selection, filtering, and saving, while preserving system-level security guarantees and a consistent asynchronous programming model.
